...
"""
马年新春祝福 - 极简完整版
版本：v1.7.4-simple-full
功能：祝福语展示、点击复制、模拟分享（控制台输出）
"""
import kivy
...
class MainScreen(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # 节日切换按钮行
        self.fest_btns = BoxLayout(size_hint_y=None, height=dp(50), spacing=5)
        ...
        self.random_btn = Button(text='随机祝福', background_color=get_color_from_hex('#8B4513'))
        self.random_btn.bind(on_press=lambda x: self.switch_festival('随机祝福'))
        self.fest_btns.add_widget(self.spring_btn)
        self.fest_btns.add_widget(self.lantern_btn)
        self.fest_btns.add_widget(self.random_btn)
        self.add_widget(self.fest_btns)

        # 分类按钮行
        self.cat_layout = BoxLayout(size_hint_y=None, height=dp(50), spacing=5)
        self.add_widget(self.cat_layout)
        ...
        # 分享按钮
        self.share_btn = Button(text='发给微信好友', size_hint_y=None, height=dp(50),
    ...
    def switch_festival(self, fest):
        self.current_festival = fest
        # 更新节日按钮颜色
        self.spring_btn.background_color = get_color_from_hex('#DAA520' if fest=='春节祝福' else '#8B4513')
        self.lantern_btn.background_color = get_color_from_hex('#DAA520' if fest=='元宵节祝福' else '#8B4513')
        self.random_btn.background_color = get_color_from_hex('#DAA520' if fest=='随机祝福' else '#8B4513')
    ...
    def update_category_buttons(self):
        ...
        for cat in self.category_list:
            btn = Button(text=cat, background_color=get_color_from_hex('#DAA520' if cat==self.current_category else '#8B4513'))
    ...
    def get_current_blessings(self):
        if self.current_festival == '春节祝福':
            return BLESSINGS_SPRING[self.current_category]
        elif self.current_festival == '元宵节祝福':
            return BLESSINGS_LANTERN[self.current_category]
        else:
            return BLESSINGS_RANDOM[self.current_category]

    def show_current_page(self):
        self.list_layout.clear_widgets()
        for text in self.get_current_blessings():
            btn = Button(text=text, size_hint_y=None, height=dp(80),
                         background_normal='', background_color=(1,1,1,0.9),
                         color=(0,0,0,1), halign='left', valign='top',
                         padding=(dp(10), dp(5)))
            btn.bind(width=lambda *x, b=btn: setattr(b, 'text_size', (b.width - dp(20), None)),
    ...
    def on_copy(self, instance):
        ...
        if self.selected_item and self.selected_item != instance:
            ...
            self.selected_item.color = (0,0,0,1)
    ...
    def share_blessings(self, instance):
        if self.last_copied_text:
            share_text(self.last_copied_text)
            show_toast('分享已启动')
        else:
            show_toast('请先选择一条祝福')
