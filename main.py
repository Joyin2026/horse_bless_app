...
# ---------- 全局常量（用于占位） ----------
APP_VERSION = "v1.7.4-test"

# ---------- 占位函数（临时，避免因缺少 jnius 崩溃） ----------
...
class GuideOverlay(FloatLayout):
    ...
    def create_step_ui(self):
        ...
        bubble_label = Label(text=text, color=(0,0,0,1), size_hint_y=0.6,
                             halign='center', valign='middle')  # 移除 font_name
        ...
        if self.current_step == len(self.steps) - 1:
            ...
            chk_label = Label(text='下次不再显示', color=(0,0,0,1), size_hint_x=0.7)  # 移除 font_name
            ...
            next_btn = Button(text='完成', size_hint_x=0.4, background_color=(0.5,0.1,0.1,1), color=(1,1,1,1))
            next_btn.bind(on_press=lambda x: self.finish(chk.active))
        else:
            next_btn = Button(text='下一步', size_hint_x=1, background_color=(0.5,0.1,0.1,1), color=(1,1,1,1))
...
class MainScreen(Screen):
    def __init__(self, **kwargs):
        ...
        top_img = Image(source='', color=(1,0,0,1))  # 红色占位
    ...
    def show_about_popup(self, instance):
        ...
        for line in info_texts:
            lbl = Label(text=line, color=(0,0,0,1),
                        halign='left', valign='middle', size_hint_y=None, height=dp(25))  # 移除 font_name
...
class BlessApp(App):
    def build(self):
        ...
        # Window.size = Window.system_size  # 可能导致问题，改为自动
